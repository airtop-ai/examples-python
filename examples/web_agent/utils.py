...
def retrieve_history(TARGET_URL: str, prompt: str):
    return dataframe_from_query_dev(
        f"SELECT * from results where (target_url='{TARGET_URL}' and prompt='{prompt}')"
    ).to_dict("records")
...
def insert_result(TARGET_URL: str, prompt: str, result: str):
    insert_statement(
        query="INSERT INTO results (target_url, prompt, result) VALUES (?, ?, ?)",
        parameters=(TARGET_URL, prompt, result),
    )


def insert_comparison(TARGET_URL: str, prompt: str, result: str):
    insert_statement(
        query="INSERT INTO comparisons (target_url, prompt, result) VALUES (?, ?, ?)",
        parameters=(TARGET_URL, prompt, result),
    )
